        ...
        if request.task_type == "BOOK_CONTRACT_PROPOSAL":
            return ModelAdapterResult(
                ...
                output={
                    ...
                    "reader_problem": (
                        "Курс существует, но продажи зависят от разовых запусков и неясной экономики"
                    ),
                    "central_promise": (
                        "Собрать управляемую и прибыльную систему продаж онлайн-курса"
                    ),
                    "central_thesis": (
                        "Продажи становятся устойчивыми, когда спрос, обещание, доказательства и "
                        "экономика проверяются как единая система"
                    ),
                    "unique_angle": (
                        "Рассматривать курс как предложение результата, а не как набор уроков"
                    ),
                    "reader_trajectory": (
                        "От случайных запусков к диагностике и управлению всей системой продаж"
                    ),
                    "explicit_exclusions": [
