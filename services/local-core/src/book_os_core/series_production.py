...
class DefinitionPackContent(BaseModel):
    ...
    def blocking_quality_fields(self) -> list[str]:
        checks = {
        ...
        return [name for name, result in checks.items() if result != "PASS"]
...
class AdmissionChecks(BaseModel):
    ...
    def blockers(self) -> list[str]:
        ...
        for key, value in raw.items():
            if key == "conditional_blockers_resolved":
    ...
    def validate_kind(self) -> ProductionCheckpointRequest:
        if self.kind == "MID_BOOK":
            if self.progress_percent is None or not 40 <= self.progress_percent <= 60:
                raise ValueError("MID_BOOK checkpoint requires progress_percent between 40 and 60")
...
class SeriesProductionService:
    _CANON_TRANSITIONS: dict[str, set[str]] = {
        ...
        "USED_DRAFT": {"RESERVED", "USED_ACCEPTED", "CROSS_REFERENCE_ONLY", "RELEASED"},
        "CROSS_REFERENCE_ONLY": {"RESERVED", "USED_DRAFT", "USED_ACCEPTED", "RELEASED"},
    ...
    def _authority_heads(
        ...
        if project.architecture is None or project.architecture.authority_status not in {
            ...
            raise SeriesProductionGateError("current Architecture must be HUMAN-approved")
        authority = AuthorityService(engine)
        architecture_head = authority.get_head(project.architecture.authority_entity_id)
        ...
        if chapter.chapter_contract.authority_status not in {"APPROVED", "LOCKED"}:
            raise SeriesProductionGateError("current Chapter Contract must be HUMAN-approved")
        chapter_head = authority.get_head(chapter.chapter_contract.authority_entity_id)
    ...
    def create_definition_pack(
        ...
        self._human(request.actor_kind)
        ...
        try:
            ...
            with engine.begin() as connection:
                revision = int(
                    connection.execute(
                        text(
                            "SELECT COALESCE(MAX(revision),0)+1 FROM definition_packs "
                ...
                connection.execute(
                    text(
                        "INSERT INTO definition_packs(definition_id,book_id,revision,content_json,"
                        "content_hash,status,created_by,created_at) VALUES "
                        "(:definition_id,:book_id,:revision,:content_json,:content_hash,'DRAFT',"
    ...
    def get_definition_pack(self, book_id: str, definition_id: str) -> DefinitionPackView:
        engine = self._engine(book_id)
        try:
            with engine.connect() as connection:
                row = connection.execute(
                    text(
                        "SELECT * FROM definition_packs WHERE book_id=:book_id "
                        "AND definition_id=:definition_id"
                    ),
                    {"book_id": book_id, "definition_id": definition_id},
    ...
    def _definition_view(self, row: Any) -> DefinitionPackView:
        return DefinitionPackView(
            ...
            content=DefinitionPackContent.model_validate(self._loads(row["content_json"], default={})),
    ...
    def latest_approved_definition(self, book_id: str) -> DefinitionPackView | None:
        engine = self._engine(book_id)
        try:
            with engine.connect() as connection:
                row = connection.execute(
                    text(
                        "SELECT * FROM definition_packs WHERE book_id=:book_id AND status='APPROVED' "
    ...
    def approve_definition_pack(
        ...
        blockers = current.content.blocking_quality_fields()
        if blockers:
        ...
        try:
            with engine.begin() as connection:
                connection.execute(
                    text(
                        "UPDATE definition_packs SET status='APPROVED',approved_by=:approved_by,"
                        "approved_at=:approved_at WHERE definition_id=:definition_id AND status='DRAFT'"
                    ),
                    {
    ...
    def create_production_contract(
        ...
        engine = self._engine(book_id)
        try:
            architecture_head, chapter_head = self._authority_heads(engine, book_id, chapter_id)
            assert chapter_head is not None
            payload = request.content.model_dump(mode="json")
            digest = content_hash(cast(dict[str, JSONValue], payload))
            production_contract_id = new_ulid()
            now = utc_now()
            with engine.begin() as connection:
                connection.execute(
                    text(
                        "INSERT INTO chapter_production_contracts("
                        "production_contract_id,book_id,chapter_id,architecture_revision_id,"
                        "architecture_revision_hash,chapter_contract_revision_id,"
                        "chapter_contract_revision_hash,content_json,content_hash,status,created_by,created_at) "
                        "VALUES (:id,:book_id,:chapter_id,:architecture_revision_id,:architecture_hash,"
                        ":chapter_revision_id,:chapter_hash,:content_json,:content_hash,'DRAFT',"
                        ":created_by,:created_at)"
                    ),
                    {
                        ...
                        "architecture_hash": architecture_head.revision_hash,
                        "chapter_revision_id": chapter_head.revision_id,
                        "chapter_hash": chapter_head.revision_hash,
                        "content_json": self._json(payload),
                        "content_hash": digest,
                        "created_by": request.actor,
                        "created_at": now,
            ...
            return self.get_production_contract(book_id, chapter_id, production_contract_id)
        finally:
            engine.dispose()

    def _production_contract_view(self, row: Any) -> ChapterProductionContractView:
        return ChapterProductionContractView(
            ...
            content=ChapterProductionContractContent.model_validate(
                self._loads(row["content_json"], default={})
            ),
            status=str(row["status"]),
            created_by=str(row["created_by"]),
            created_at=str(row["created_at"]),
    ...
    def get_production_contract(
        self, book_id: str, chapter_id: str, production_contract_id: str
    ) -> ChapterProductionContractView:
        engine = self._engine(book_id)
        try:
            with engine.connect() as connection:
                row = connection.execute(
                    text(
                        "SELECT * FROM chapter_production_contracts WHERE book_id=:book_id "
                        "AND chapter_id=:chapter_id AND production_contract_id=:id"
                    ),
                    {"book_id": book_id, "chapter_id": chapter_id, "id": production_contract_id},
                ).mappings().one_or_none()
            if row is None:
                raise SeriesProductionError("Chapter Production Contract not found")
        ...
        try:
            with engine.connect() as connection:
                row = connection.execute(
                    text(
                        "SELECT * FROM chapter_production_contracts WHERE book_id=:book_id "
                        "AND chapter_id=:chapter_id AND status='APPROVED' ORDER BY created_at DESC LIMIT 1"
                    ),
    ...
    def approve_production_contract(
        ...
        self._human(request.actor_kind)
        current = self.get_production_contract(book_id, chapter_id, production_contract_id)
        if current.status != "DRAFT":
            raise SeriesProductionGateError("only a DRAFT Chapter Production Contract can be approved")
        blockers = current.content.blocking_quality_fields()
        if blockers:
            raise SeriesProductionGateError(
                "Chapter Production Contract requires REWORK: " + ", ".join(blockers)
            )
        engine = self._engine(book_id)
        try:
            architecture_head, chapter_head = self._authority_heads(engine, book_id, chapter_id)
            assert chapter_head is not None
            if (
                current.architecture_revision_id != architecture_head.revision_id
                or current.architecture_revision_hash != architecture_head.revision_hash
                or current.chapter_contract_revision_id != chapter_head.revision_id
                or current.chapter_contract_revision_hash != chapter_head.revision_hash
            ):
                raise SeriesProductionGateError(
            ...
            with engine.begin() as connection:
                connection.execute(
                    text(
                        ...
                        "approved_by=:actor,approved_at=:approved_at WHERE production_contract_id=:id "
                        "AND status='DRAFT'"
                    ),
                    {"actor": request.actor, "approved_at": utc_now(), "id": production_contract_id},
                )
            return self.get_production_contract(book_id, chapter_id, production_contract_id)
        finally:
            engine.dispose()
    ...
    def create_canon_asset(self, request: SeriesCanonAssetCreateRequest) -> SeriesCanonAssetView:
        ...
        try:
            ...
            with engine.begin() as connection:
                connection.execute(
                    text(
                        "INSERT INTO series_canon_assets(asset_id,series_profile_id,book_id,asset_type,"
                        "asset_key,description,status,source_revision_id,source_revision_hash,"
                        "provenance_json,created_by,created_at,updated_at) VALUES "
                        "(:asset_id,:series_profile_id,:book_id,:asset_type,:asset_key,:description,"
                        ":status,:source_revision_id,:source_revision_hash,:provenance_json,:created_by,"
    ...
    def get_canon_asset(self, book_id: str, asset_id: str) -> SeriesCanonAssetView:
        ...
        try:
            with engine.connect() as connection:
                row = connection.execute(
                    text("SELECT * FROM series_canon_assets WHERE asset_id=:asset_id"),
                    {"asset_id": asset_id},
                ).mappings().one_or_none()
            if row is None:
            ...
            return SeriesCanonAssetView(
                ...
                source_revision_id=cast(str | None, row["source_revision_id"]),
                source_revision_hash=cast(str | None, row["source_revision_hash"]),
                provenance=cast(dict[str, Any], self._loads(row["provenance_json"], default={})),
                created_by=str(row["created_by"]),
                created_at=str(row["created_at"]),
    ...
    def transition_canon_asset(
        self, book_id: str, asset_id: str, request: SeriesCanonTransitionRequest
    ) -> SeriesCanonAssetView:
        current = self.get_canon_asset(book_id, asset_id)
        if request.status == current.status:
            return current
        allowed = self._CANON_TRANSITIONS[current.status]
        if request.status not in allowed:
            raise SeriesProductionGateError(
                f"invalid Series Canon transition: {current.status} -> {request.status}"
            )
        provenance = {
            **current.provenance,
            "last_transition": {
        ...
        try:
            with engine.begin() as connection:
                connection.execute(
                    text(
                        "UPDATE series_canon_assets SET status=:status,provenance_json=:provenance,"
                        "updated_at=:updated_at WHERE asset_id=:asset_id"
                    ),
                    {
                        "status": request.status,
    ...
    def record_uniqueness(
        self, book_id: str, chapter_id: str, request: UniquenessEvidenceRequest
        ...
        try:
            ...
            with engine.begin() as connection:
                connection.execute(
                    text(
                        "INSERT INTO book_uniqueness_ledger(ledger_id,book_id,chapter_id,"
                        "architecture_revision_id,architecture_revision_hash,chapter_contract_revision_id,"
                        "chapter_contract_revision_hash,status,evidence_json,actor_kind,actor,created_at) "
                        "VALUES (:ledger_id,:book_id,:chapter_id,:architecture_revision_id,"
                        ":architecture_hash,:chapter_revision_id,:chapter_hash,:status,:evidence_json,"
    ...
    def _latest_uniqueness(
        self, engine: Engine, book_id: str, chapter_id: str
    ) -> Any | None:
        with engine.connect() as connection:
            return connection.execute(
                text(
                    "SELECT * FROM book_uniqueness_ledger WHERE book_id=:book_id "
                    "AND chapter_id=:chapter_id ORDER BY created_at DESC LIMIT 1"
                ),
                {"book_id": book_id, "chapter_id": chapter_id},
            ).mappings().one_or_none()
    ...
    def admit_chapter(
        ...
        if explicit_blockers:
            raise SeriesProductionGateError(
                "Chapter admission checks require REWORK: " + ", ".join(explicit_blockers)
            )
        ...
        try:
            architecture_head, chapter_head = self._authority_heads(engine, book_id, chapter_id)
            assert chapter_head is not None
            production_contract = self.latest_approved_production_contract(book_id, chapter_id)
            if production_contract is None:
                raise SeriesProductionGateError(
                    "current approved Chapter Production Contract is required"
                )
            if (
                production_contract.architecture_revision_id != architecture_head.revision_id
                or production_contract.architecture_revision_hash != architecture_head.revision_hash
                or production_contract.chapter_contract_revision_id != chapter_head.revision_id
                or production_contract.chapter_contract_revision_hash != chapter_head.revision_hash
            ):
                raise SeriesProductionGateError("Chapter Production Contract is stale")
            uniqueness = self._latest_uniqueness(engine, book_id, chapter_id)
            if uniqueness is None or uniqueness["status"] != "PASS":
                raise SeriesProductionGateError("current uniqueness / overlap evidence must PASS")
            if (
                uniqueness["architecture_revision_id"] != architecture_head.revision_id
                or uniqueness["architecture_revision_hash"] != architecture_head.revision_hash
                or uniqueness["chapter_contract_revision_id"] != chapter_head.revision_id
                or uniqueness["chapter_contract_revision_hash"] != chapter_head.revision_hash
            ):
                raise SeriesProductionGateError("uniqueness evidence is stale")
            admission_id = new_ulid()
            now = utc_now()
            with engine.begin() as connection:
                connection.execute(
                    text(
                        "INSERT INTO chapter_admissions(admission_id,book_id,chapter_id,definition_id,"
                        "definition_hash,architecture_revision_id,architecture_revision_hash,"
                        "chapter_contract_revision_id,chapter_contract_revision_hash,"
                        "production_contract_id,production_contract_hash,checks_json,status,actor_kind,"
                        "actor,reason,created_at) VALUES (:admission_id,:book_id,:chapter_id,"
                        ":definition_id,:definition_hash,:architecture_revision_id,:architecture_hash,"
                        ":chapter_revision_id,:chapter_hash,:production_contract_id,"
                        ":production_contract_hash,:checks_json,'WRITING_ALLOWED',:actor_kind,:actor,"
                        ":reason,:created_at)"
                    ),
                    {
                        ...
                        "chapter_revision_id": chapter_head.revision_id,
                        "chapter_hash": chapter_head.revision_hash,
                        "production_contract_id": production_contract.production_contract_id,
                        "production_contract_hash": production_contract.content_hash,
                        "checks_json": self._json(request.checks.model_dump(mode="json")),
                        "actor_kind": request.actor_kind,
                        "actor": request.actor,
    ...
    def _latest_checkpoint(self, engine: Engine, book_id: str, kind: str) -> Any | None:
        with engine.connect() as connection:
            return connection.execute(
                text(
    ...
    def _current_character_count(self, engine: Engine, book_id: str) -> int:
        with engine.connect() as connection:
        ...
        total = 0
        for row in rows:
            payload = self._loads(row[0], default={})
            if isinstance(payload, dict) and isinstance(payload.get("text"), str):
                total += len(payload["text"])
        return total

    def _mid_book_blocker(self, engine: Engine, book_id: str) -> str | None:
        with engine.connect() as connection:
            target = connection.execute(
                text("SELECT target_characters FROM book_context_settings WHERE book_id=:book_id"),
                {"book_id": book_id},
            ).scalar_one_or_none()
    ...
    def admission_status(self, book_id: str, chapter_id: str) -> ChapterAdmissionStatusView:
        blockers: list[str] = []
        definition = self.latest_approved_definition(book_id)
        if definition is None:
        ...
        try:
            ...
            with engine.connect() as connection:
                admission = connection.execute(
                    text(
                        "SELECT * FROM chapter_admissions WHERE book_id=:book_id AND chapter_id=:chapter_id "
                        "ORDER BY created_at DESC LIMIT 1"
                    ),
                    {"book_id": book_id, "chapter_id": chapter_id},
                ).mappings().one_or_none()
            if admission is None:
                ...
                if definition is None or (
                    ...
                    blockers.append("chapter admission is stale against Definition Pack")
                if (
                    admission["architecture_revision_id"] != architecture_head.revision_id
                    or admission["architecture_revision_hash"] != architecture_head.revision_hash
                    or admission["chapter_contract_revision_id"] != chapter_head.revision_id
                    or admission["chapter_contract_revision_hash"] != chapter_head.revision_hash
                ):
                    blockers.append("chapter admission is stale against current authority")
                if production_contract is None or (
                    admission["production_contract_id"]
                    != production_contract.production_contract_id
                    or admission["production_contract_hash"] != production_contract.content_hash
                ):
                    blockers.append("chapter admission is stale against production contract")
            ...
            return ChapterAdmissionStatusView(
                ...
                blockers=blockers,
                admission_id=(str(admission["admission_id"]) if admission is not None else None),
    ...
    def require_writing_allowed(self, book_id: str, chapter_id: str) -> None:
    ...
    def record_checkpoint(
        self, book_id: str, request: ProductionCheckpointRequest
        ...
        try:
            ...
            with engine.begin() as connection:
                connection.execute(
                    text(
                        "INSERT INTO production_checkpoints(checkpoint_id,book_id,kind,progress_percent,"
                        "status,findings_json,actor_kind,actor,executor_identity,snapshot_hash,created_at) "
                        "VALUES (:checkpoint_id,:book_id,:kind,:progress_percent,:status,:findings_json,"
                        ":actor_kind,:actor,:executor_identity,:snapshot_hash,:created_at)"
                    ),
                    {
                        ...
                        "findings_json": self._json(
                            {"independent": request.independent, "findings": request.findings}
                        ),
                        "actor_kind": request.actor_kind,
                        "actor": request.actor,
    ...
    def adversarial_review_gate(self, book_id: str) -> tuple[bool, list[str]]:
        engine = self._engine(book_id)
        try:
            latest = self._latest_checkpoint(engine, book_id, "ADVERSARIAL_REVIEW")
            if latest is None:
                return False, ["independent Adversarial Review missing"]
            if latest["status"] == "BLOCKING":
                return False, ["Adversarial Review has unresolved BLOCKING findings"]
            payload = self._loads(latest["findings_json"], default={})
            if not isinstance(payload, dict) or payload.get("independent") is not True:
                return False, ["Adversarial Review independence evidence missing"]
            return True, []
        finally:
            engine.dispose()
        ...
        try:
            with engine.connect() as connection:
                master = connection.execute(
                    ...
                    {"book_id": book_id, "master_id": request.master_id},
                ).mappings().one_or_none()
            if master is None or master["status"] != "LOCKED":
                raise SeriesProductionGateError("exact LOCKED Literary Master is required")
            if master["manifest_hash"] != request.master_manifest_hash:
                raise SeriesProductionGateError("Literary Master manifest hash mismatch")
            overlap = set(request.used_asset_ids) & set(request.released_asset_ids)
            if overlap:
                raise SeriesProductionGateError("an asset cannot be both used and released")
            for asset_id in request.used_asset_ids:
                asset = self.get_canon_asset(book_id, asset_id)
                if asset.series_profile_id != request.series_profile_id:
                    raise SeriesProductionGateError("Series Canon asset belongs to another series")
                if asset.status == "USED_ACCEPTED":
                    continue
            ...
            with engine.begin() as connection:
                connection.execute(
                    text(
                        "INSERT INTO series_closures(closure_id,book_id,series_profile_id,master_id,"
                        "master_manifest_hash,used_asset_ids_json,released_asset_ids_json,evidence_json,"
                        "human_actor,created_at) VALUES (:closure_id,:book_id,:series_profile_id,"
                        ":master_id,:manifest_hash,:used_json,:released_json,:evidence_json,:human_actor,"
                        ":created_at)"
                    ),
                    {
                        ...
                        "manifest_hash": request.master_manifest_hash,
                        "used_json": self._json(request.used_asset_ids),
                        "released_json": self._json(request.released_asset_ids),
                        "evidence_json": self._json(request.evidence),
