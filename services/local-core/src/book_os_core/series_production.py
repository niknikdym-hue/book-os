...
class DefinitionPackContent(BaseModel):
    ...
    def blockers(self) -> list[str]:
        ...
        return [name for name, result in gates.items() if result != "PASS"]
...
class SeriesProductionService:
    _CANON_TRANSITIONS: dict[str, set[str]] = {
        "PLANNED": {"RESERVED", "USED_DRAFT", "CROSS_REFERENCE_ONLY", "RELEASED"},
    ...
    def _definition_view(self, row: Any) -> DefinitionPackView:
        return DefinitionPackView(
            ...
            content=DefinitionPackContent.model_validate(self._loads(row["content_json"], {})),
    ...
    def create_definition_pack(
        ...
        try:
            ...
            with engine.begin() as connection:
                ...
                connection.execute(
                    text(
                        "INSERT INTO definition_packs(definition_id,book_id,revision,content_json,content_hash,"
                        "status,created_by,created_at) VALUES (:id,:book_id,:revision,:content,:hash,'DRAFT',"
                        ":actor,:created_at)"
                    ),
                    {
                        "id": definition_id,
                        ...
                        "hash": digest,
                        "actor": f"{request.actor_kind}:{request.actor}",
                        "created_at": now,
                    },
                )
    ...
    def get_definition_pack(self, book_id: str, definition_id: str) -> DefinitionPackView:
        ...
        try:
            row = self._row(
                engine,
                "SELECT * FROM definition_packs WHERE book_id=:book_id AND definition_id=:id",
                {"book_id": book_id, "id": definition_id},
            )
            if row is None:
    ...
    def approve_definition_pack(
        ...
        try:
            with engine.begin() as connection:
                connection.execute(
                    text(
                        "UPDATE definition_packs SET status='APPROVED',approved_by=:actor,approved_at=:at "
                        "WHERE definition_id=:id AND status='DRAFT'"
                    ),
                    {"actor": request.actor, "at": utc_now(), "id": definition_id},
                )
            return self.get_definition_pack(book_id, definition_id)
        finally:
            engine.dispose()
    ...
    def create_production_contract(
        self, book_id: str, chapter_id: str, request: ChapterProductionContractCreateRequest
    ) -> ChapterProductionContractView:
        engine = self._engine(book_id)
        try:
            architecture, chapter = self._heads(engine, book_id, chapter_id)
            ...
            with engine.begin() as connection:
                connection.execute(
                    text(
                        "INSERT INTO chapter_production_contracts(production_contract_id,book_id,chapter_id,"
                        ...
                        "VALUES (:id,:book_id,:chapter_id,:arid,:arhash,:crid,:crhash,:content,:hash,'DRAFT',"
                        ":actor,:at)"
                    ),
                    {
                        "id": contract_id,
                        ...
                        "arid": architecture.revision_id,
                        "arhash": architecture.revision_hash,
                        "crid": chapter.revision_id,
                        "crhash": chapter.revision_hash,
                        "content": self._json(payload),
                        "hash": digest,
                        "actor": f"{request.actor_kind}:{request.actor}",
                        "at": utc_now(),
            ...
            return self.get_production_contract(book_id, chapter_id, contract_id)
        finally:
            engine.dispose()

    def get_production_contract(
        self, book_id: str, chapter_id: str, contract_id: str
    ) -> ChapterProductionContractView:
        engine = self._engine(book_id)
        try:
            row = self._row(
                engine,
                "SELECT * FROM chapter_production_contracts WHERE book_id=:book_id AND chapter_id=:chapter_id "
                "AND production_contract_id=:id",
                {"book_id": book_id, "chapter_id": chapter_id, "id": contract_id},
            )
            if row is None:
    ...
    def approve_production_contract(
        ...
        try:
            ...
            bound = (
                ...
                current.architecture_revision_hash,
                current.chapter_contract_revision_id,
                current.chapter_contract_revision_hash,
            )
            current_authority = (
            ...
            with engine.begin() as connection:
                connection.execute(
                    text(
                        "UPDATE chapter_production_contracts SET status='APPROVED',approved_by=:actor,"
                        "approved_at=:at WHERE production_contract_id=:id AND status='DRAFT'"
                    ),
                    {"actor": request.actor, "at": utc_now(), "id": contract_id},
                )
            return self.get_production_contract(book_id, chapter_id, contract_id)
        finally:
            engine.dispose()
    ...
    def create_canon_asset(self, request: SeriesCanonAssetCreateRequest) -> SeriesCanonAssetView:
        ...
        try:
            ...
            with engine.begin() as connection:
                connection.execute(
                    text(
                        "INSERT INTO series_canon_assets(asset_id,series_profile_id,book_id,asset_type,asset_key,"
                        "description,status,source_revision_id,source_revision_hash,provenance_json,created_by,"
                        "created_at,updated_at) VALUES (:id,:series_id,:book_id,:type,:key,:description,:status,"
                        ":revision_id,:revision_hash,:provenance,:actor,:at,:at)"
                    ),
                    {
                        "id": asset_id,
                        "series_id": request.series_profile_id,
                        ...
                        "revision_hash": request.source_revision_hash,
                        "provenance": self._json(request.provenance),
                        "actor": request.actor,
                        "at": now,
                    },
                )
    ...
    def get_canon_asset(self, book_id: str, asset_id: str) -> SeriesCanonAssetView:
        ...
        try:
            ...
            return SeriesCanonAssetView(
                ...
                provenance=cast(dict[str, Any], self._loads(row["provenance_json"], {})),
                created_by=str(row["created_by"]),
                created_at=str(row["created_at"]),
    ...
    def transition_canon_asset(
        ...
        try:
            with engine.begin() as connection:
                connection.execute(
                    text(
                        "UPDATE series_canon_assets SET status=:status,provenance_json=:provenance,updated_at=:at "
                        "WHERE asset_id=:id"
                    ),
                    {
                        "status": request.status,
    ...
    def record_uniqueness(
        ...
        try:
            ...
            with engine.begin() as connection:
                connection.execute(
                    text(
                        "INSERT INTO book_uniqueness_ledger(ledger_id,book_id,chapter_id,architecture_revision_id,"
                        "architecture_revision_hash,chapter_contract_revision_id,chapter_contract_revision_hash,"
                        "status,evidence_json,actor_kind,actor,created_at) VALUES (:id,:book_id,:chapter_id,:arid,"
                        ":arhash,:crid,:crhash,:status,:evidence,:actor_kind,:actor,:at)"
                    ),
                    {
                        "id": ledger_id,
                        "book_id": book_id,
                        ...
                        "crhash": chapter.revision_hash,
                        "status": request.status,
                        "evidence": self._json(request.evidence),
            ...
            return UniquenessEvidenceView(
                ...
                architecture_revision_hash=architecture.revision_hash,
                chapter_contract_revision_id=chapter.revision_id,
                chapter_contract_revision_hash=chapter.revision_hash,
                status=request.status,
                evidence=request.evidence,
                actor_kind=request.actor_kind,
                actor=request.actor,
    ...
    def _latest_uniqueness(self, engine: Engine, book_id: str, chapter_id: str) -> Any | None:
        return self._row(
            engine,
            "SELECT * FROM book_uniqueness_ledger WHERE book_id=:book_id AND chapter_id=:chapter_id "
    ...
    def admit_chapter(
        ...
        try:
            ...
            if (
                ...
                contract.chapter_contract_revision_id,
                contract.chapter_contract_revision_hash,
            ) != (
                architecture.revision_id,
                architecture.revision_hash,
                chapter.revision_id,
            ...
            if (
                ...
                uniqueness["architecture_revision_hash"],
                uniqueness["chapter_contract_revision_id"],
                uniqueness["chapter_contract_revision_hash"],
            ) != (
                architecture.revision_id,
                architecture.revision_hash,
                chapter.revision_id,
            ...
            with engine.begin() as connection:
                connection.execute(
                    text(
                        "INSERT INTO chapter_admissions(admission_id,book_id,chapter_id,definition_id,definition_hash,"
                        "architecture_revision_id,architecture_revision_hash,chapter_contract_revision_id,"
                        "chapter_contract_revision_hash,production_contract_id,production_contract_hash,checks_json,"
                        "status,actor_kind,actor,reason,created_at) VALUES (:id,:book_id,:chapter_id,:definition_id,"
                        ":definition_hash,:arid,:arhash,:crid,:crhash,:pcid,:pchash,:checks,'WRITING_ALLOWED',"
                        ":actor_kind,:actor,:reason,:at)"
                    ),
                    {
                        "id": new_ulid(),
                        "book_id": book_id,
                        ...
                        "crid": chapter.revision_id,
                        "crhash": chapter.revision_hash,
                        "pcid": contract.production_contract_id,
                        "pchash": contract.content_hash,
                        "checks": self._json(request.checks.model_dump(mode="json")),
                        "actor_kind": request.actor_kind,
                        "actor": request.actor,
    ...
    def _mid_book_blocker(self, engine: Engine, book_id: str) -> str | None:
        ...
        if not isinstance(target, int) or target <= 0:
            return None
        if self._current_character_count(engine, book_id) / target * 100 < 40:
            return None
        checkpoint = self._row(
            engine,
            "SELECT * FROM production_checkpoints WHERE book_id=:book_id AND kind='MID_BOOK' "
            "ORDER BY created_at DESC LIMIT 1",
            {"book_id": book_id},
        )
        ...
        try:
            ...
            if admission is None:
                ...
                if definition is None or (
                    admission["definition_id"],
                    admission["definition_hash"],
                ) != (definition.definition_id, definition.content_hash):
                    blockers.append("chapter admission is stale against Definition Pack")
                if (
                    admission["architecture_revision_id"],
                    admission["architecture_revision_hash"],
                    admission["chapter_contract_revision_id"],
                    admission["chapter_contract_revision_hash"],
                ) != (
                    architecture.revision_id,
                    architecture.revision_hash,
                    chapter.revision_id,
                ...
                if contract is None or (
                    ...
                    admission["production_contract_hash"],
                ) != (contract.production_contract_id, contract.content_hash):
    ...
    def require_writing_allowed(self, book_id: str, chapter_id: str) -> None:
        ...
        if not status.writing_allowed:
            raise SeriesProductionGateError("WRITING_NOT_ALLOWED: " + "; ".join(status.blockers))
    ...
    def record_checkpoint(
        ...
        try:
            ...
            now = utc_now()
            findings = {"independent": request.independent, "findings": request.findings}
            with engine.begin() as connection:
                connection.execute(
                    text(
                        "INSERT INTO production_checkpoints(checkpoint_id,book_id,kind,progress_percent,status,"
                        "findings_json,actor_kind,actor,executor_identity,snapshot_hash,created_at) VALUES "
                        "(:id,:book_id,:kind,:progress,:status,:findings,:actor_kind,:actor,:executor,:snapshot,:at)"
                    ),
                    {
                        "id": checkpoint_id,
                        "book_id": book_id,
                        ...
                        "status": request.status,
                        "findings": self._json(findings),
                        "actor_kind": request.actor_kind,
                        "actor": request.actor,
                        "executor": request.executor_identity,
    ...
    def adversarial_review_gate(self, book_id: str) -> tuple[bool, list[str]]:
        ...
        try:
            latest = self._row(
                engine,
                "SELECT * FROM production_checkpoints WHERE book_id=:book_id AND kind='ADVERSARIAL_REVIEW' "
                "ORDER BY created_at DESC LIMIT 1",
                {"book_id": book_id},
            )
            if latest is None:
    ...
    def close_series_book(self, book_id: str, request: SeriesClosureRequest) -> SeriesClosureView:
        ...
        try:
            ...
            if set(request.used_asset_ids) & set(request.released_asset_ids):
            ...
            for asset_id in request.used_asset_ids:
                self.transition_canon_asset(
                    ...
                    SeriesCanonTransitionRequest(
                        ...
                        actor=request.actor,
                        evidence={"master_id": request.master_id, **request.evidence},
                    ),
                )
            ...
            with engine.begin() as connection:
                connection.execute(
                    text(
                        ...
                        "human_actor,created_at) VALUES (:id,:book_id,:series_id,:master_id,:hash,:used,:released,"
                        ":evidence,:actor,:at)"
                    ),
                    {
                        "id": closure_id,
                        ...
                        "hash": request.master_manifest_hash,
                        "used": self._json(request.used_asset_ids),
                        "released": self._json(request.released_asset_ids),
                        "evidence": self._json(request.evidence),
                        "actor": request.actor,
                        "at": now,
                    },
