...
3. **Unknown is not valid.** If movement between different locations requires a duration and no accepted rule exists, feasibility is unresolved and blocks a deterministic PASS.
4. **Knowledge is causal.** A character may use a fact only if it is initial knowledge or was acquired at/before the use event.
5. **Clues have lifecycle.** A material clue has a cause/origin, reader exposure where required, dependencies and payoff/recontextualization.
6. **Fair play is profile-aware.** In `REQUIRED` mode, a decisive clue without reader exposure is BLOCKING; in `EXPECTED` mode it is MAJOR; `RELAXED` does not make broken references/order valid.
7. **No opaque score.** Output is a list of exact findings; any BLOCKING finding prevents case-integrity PASS.
8. **Real-world facts remain research authority.** MYS-02 consumes explicit durations/rules but does not replace Fiction Research & Realism.
